...
function sign(u) {
    const payload = { id: u.id, nombre: u.nombre, email: u.email, rol: u.rol, saldo: u.saldo ?? 0 };
...
export async function register(req, res, next) {
    try {
        ...
        if (exists.rowCount) return res.status(409).json({ error: 'Email o usuario ya existe' });

        const hash = await bcrypt.hash(contrasena, 10);
        const { rows } = await pool.query(
            `INSERT INTO usuario (nombre, email, telefono, contrasena)
       VALUES ($1,$2,$3,$4)
       RETURNING id, nombre, email, rol, saldo`,
...
export async function me(req, res) {
    // si usas requireAuth que popula req.user
    return res.json(req.user);
}

export async function forgotPassword(req, res, next) {
    try {
        const { emailOrUser } = req.body; // puede ser email o usuario
       ...
       WHERE lower(email)=lower($1) OR lower(nombre)=lower($1) LIMIT 1`,
        ...
        const u = rows[0];
        if (!u) return res.json({ ok: true }); // no revelar si existe o no
        ...
        const resetLink = `${process.env.PUBLIC_BASE_URL || 'http://localhost:5173'}/reset?token=${token}`;
        console.log('[reset-link]', resetLink); // en dev, a consola
...
export async function resetPassword(req, res, next) {
    try {
        ...
        if (!token || !nueva) return res.status(400).json({ error: 'Datos inválidos' });
       ...
       WHERE pr.token=$1 AND pr.used=FALSE AND pr.expires_at>NOW() LIMIT 1`,
        ...
        await pool.query('UPDATE usuario SET contrasena=$1 WHERE id=$2', [hash, r.usuario_id]);
